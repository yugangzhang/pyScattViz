...
def test_safe_collection_name_sanitizes_and_rejects_empty_names():
    assert safe_collection_name("  my project / giwaxs ") == "my_project_giwaxs"
    assert safe_collection_name("2026-2_pass-123456") == "2026-2_pass-123456"
...
def test_normalize_paths_translates_a_remote_path_through_a_mapping():
    ...
    result = normalize_paths(
        ["/nsls2/data/smi/proposals/2026-2/pass-123456/Results/giwaxs"], mappings
    )
    assert result == ["/mnt/smi/2026-2/pass-123456/Results/giwaxs"]


def test_save_and_load_a_collection_round_trip(config_home):
    written = save_collection("my project", ["/mnt/a", "/mnt/b"], "0.10 and 0.15 deg")
    assert written.name == "my_project.json"

    payload = load_collection("my project")
